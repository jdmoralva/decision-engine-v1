...
- **User Story 4 (Phase 3)**: Depends on Foundational completion - establishes admin-managed runtime and administrative visibility/detail semantics
...
- **US4**: Can start after Phase 2 - no dependency on later stories; now also owns product/workflow visibility, detail, `delete`/`retire` semantics, approval metadata in `draft`, and audit-preserving hidden-state behavior
...
- In US4, T016-T018 can run together; T019-T021 can run in parallel; T025-T026 can run in parallel; T071 can start with other US4 tests, T072 after foundational/admin contracts are stable, and T073 after T072 and frontend service updates
...
Task: "Add backend regression tests for administrative product/workflow visibility semantics, including role-gated module access, default active listings, explicit draft switching, required detail metadata, pending approval metadata in draft, and hidden-but-persisted retired/deleted artifacts in backend/tests/integration/test_engine_admin_visibility.py and backend/tests/contract/test_engine_admin_api.py"
